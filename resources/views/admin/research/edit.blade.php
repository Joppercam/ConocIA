...
<div class="container-fluid">
    <div class="row mb-4">
        <div class="col-12">
            <h1 class="h3">Editar Investigación</h1>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('admin.research.index') }}">Investigaciones</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Editar Investigación</li>
                </ol>
            </nav>
        </div>
    </div>
        ...
        <div class="card-body">
            <form action="{{ route('admin.research.update', $research) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-8">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $research->title) }}" required>
                            @error('title')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" value="{{ old('slug', $research->slug) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('summary') is-invalid @enderror" id="summary" name="summary" rows="3" required>{{ old('summary', $research->summary) }}</textarea>
                            @error('summary')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control editor @error('content') is-invalid @enderror" id="content" name="content" rows="10" required>{{ old('content', $research->content) }}</textarea>
                            @error('content')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="card mb-3">
                            ...
                            <div class="card-body">
                                <select class="form-control @error('status') is-invalid @enderror" id="status" name="status" required>
                                    <option value="draft" {{ old('status', $research->status) == 'draft' ? 'selected' : '' }}>Borrador</option>
                                    <option value="published" {{ old('status', $research->status) == 'published' ? 'selected' : '' }}>Publicado</option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                ...
                                <div class="mt-3">
                                    ...
                                    <input type="datetime-local" class="form-control @error('published_at') is-invalid @enderror" id="published_at" name="published_at" value="{{ old('published_at', $research->published_at ? $research->published_at->format('Y-m-d\TH:i') : '') }}">
                                    @error('published_at')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <div class="card mb-3">
                            ...
                            <div class="card-body">
                                <select class="form-control @error('research_type') is-invalid @enderror" id="research_type" name="research_type" required>
                                    ...
                                    <option value="paper" {{ old('research_type', $research->research_type) == 'paper' ? 'selected' : '' }}>Paper académico</option>
                                    <option value="report" {{ old('research_type', $research->research_type) == 'report' ? 'selected' : '' }}>Reporte</option>
                                    <option value="analysis" {{ old('research_type', $research->research_type) == 'analysis' ? 'selected' : '' }}>Análisis</option>
                                    <option value="study" {{ old('research_type', $research->research_type) == 'study' ? 'selected' : '' }}>Estudio</option>
                                </select>
                                @error('research_type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="card mb-3">
                            ...
                            <div class="card-body">
                                <select class="form-control tags-select @error('tags') is-invalid @enderror" id="tags" name="tags[]" multiple>
                                    @foreach($tags as $tag)
                                        <option value="{{ $tag->id }}" {{ (old('tags') && in_array($tag->id, old('tags'))) || (in_array($tag->id, $research->tags->pluck('id')->toArray())) ? 'selected' : '' }}>
                                            {{ $tag->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="card mb-3">
                            ...
                            <div class="card-body">
                                @if($research->featured_image)
                                    <div class="mb-2">
                                        <img src="{{ asset('storage/' . $research->featured_image) }}" alt="{{ $research->title }}" class="img-fluid" style="max-height: 200px;">
                                    </div>
                                @endif
                            ...
                            <div class="card-body">
                                @if($research->pdf_file)
                                    <div class="mb-2">
                                        ...
                                        <a href="{{ asset('storage/' . $research->pdf_file) }}" target="_blank">
                                            Ver PDF actual
                                        </a>
                                    </div>
                                @endif
                ...
                <div class="d-flex justify-content-end mt-4">
                    ...
                    <button type="submit" class="btn btn-primary">Actualizar investigación</button>
                </div>
            </form>
        </div>
    </div>

        ...
        <div class="row justify-content-center">
            <div class="col-lg-8 text-center">
                <h1 class="mb-0 text-uppercase fw-bold fs-4">
                    <span class="d-inline-block border-bottom border-2 pb-1">Videos de {{ $category->name }}</span>
                </h1>
                <p class="mb-0 mt-1 text-white-50 fs-6">
                    {{ $category->description ?? 'Explora nuestra colección de videos sobre ' . $category->name }}
                </p>
            </div>
        </div>
    </div>
        ...
        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
            ...
            <div class="col-12">
                <div class="alert alert-info text-center">
                    <i class="fas fa-info-circle me-2"></i> No hay videos disponibles en esta categoría.
                </div>
...
<section class="py-3 bg-white border-top">
    <div class="container">
        <div class="row">
            <div class="col-12">
                <h3 class="h5 mb-3">Otras categorías que te pueden interesar:</h3>
                <div class="d-flex flex-wrap gap-2">
                    @php
                        $otherCategories = \App\Models\VideoCategory::where('id', '!=', $category->id)
                            ->withCount('videos')
                    ...
                    <a href="{{ route('videos.category', $otherCategory->id) }}" class="btn btn-outline-secondary mb-2 rounded-pill">
                        <i class="fas fa-folder me-1"></i> {{ $otherCategory->name }}
                        <span class="badge bg-secondary rounded-pill ms-1">{{ $otherCategory->videos_count }}</span>
                    </a>
                    @endforeach
                </div>
            </div>

    ...
    <div class="row g-4">
        <div class="col-lg-9">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    <a href="{{ route('analisis.show', $analysis->slug) }}" class="text-decoration-none d-block h-100">
                        <div class="profundiza-card h-100 p-4">
                            <span class="badge mb-2" style="background:rgba(56,182,255,.1);color:#0369a1;font-size:.68rem;">{{ strtoupper($analysis->category ?? 'ANÁLISIS') }}</span>
                            <h6 class="fw-bold mb-2" style="color:#0f172a;font-size:.95rem;line-height:1.4;">{{ $analysis->title }}</h6>
                            <p style="color:#64748b;font-size:.83rem;line-height:1.55;margin-bottom:.75rem;">{{ Str::limit($analysis->excerpt, 110) }}</p>
                            <div class="d-flex justify-content-between" style="font-size:.75rem;color:#94a3b8;">
                                <span><i class="fas fa-clock me-1"></i>{{ $analysis->reading_time ?? 10 }} min</span>
                                @if($analysis->published_at)<span>{{ $analysis->published_at->locale('es')->isoFormat('D MMM') }}</span>@endif
                            </div>
                        </div>
                    </a>
                </div>
